...
export async function GET(request: Request) {
  ...
  const vibrantBackground =
    data?.mainImage?.asset?.metadata?.palette?.vibrant?.background ?? "#3B82F6";
  const darkVibrantBackground =
    data?.mainImage?.asset?.metadata?.palette?.darkVibrant?.background ?? "#3B82F6";
        ...
        <div tw="flex flex-row w-full h-full relative">
          ...
          {data.mainImage && (
            <div tw="flex w-[500px] h-[630px] overflow-hidden">
              ...
              <img
                src={urlFor(data.mainImage).width(500).height(630).url()}
                alt=""
